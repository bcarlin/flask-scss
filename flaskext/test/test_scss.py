...
class ScssTest(unittest.TestCase):
    ...
    def test_update_scss_update_only_changed_scss_files(self):
        ...
        os.utime(scss_older_path, (time.time()-10, time.time()-10))
        os.utime(css_must_not_change_path, (time.time()-5, time.time()-5))
        os.utime(scss_newer_path, (time.time()-5, time.time()-5))
        os.utime(css_must_be_compiled_path, (time.time()-10, time.time()-10))
